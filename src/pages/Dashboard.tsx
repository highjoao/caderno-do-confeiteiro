...
import { DollarSign, TrendingDown, Building2, TrendingUp, Target, BarChart3, CalendarDays, ShoppingBag } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: gastosData } = await supabase
        ...
        .gte("data", start)
        .lte("data", end);
      setGastosTotal((gastosData || []).reduce((s, g) => s + toNumber(g.valor), 0));
  ...
  const lucro = faturamentoTotal - gastosTotal - custosFixosTotal;
  const diasLancados = fechamentos.length || 1;
  const ticketMedio = faturamentoTotal / diasLancados;
  ...
  const cards = [
    ...
    { title: "Gastos", value: formatCurrency(gastosTotal), icon: TrendingDown, color: "text-destructive" },
    { title: "Custos Fixos", value: formatCurrency(custosFixosTotal), icon: Building2, color: "text-warning" },
    { title: "Lucro Estimado", value: formatCurrency(lucro), icon: TrendingUp, color: lucro >= 0 ? "text-success" : "text-destructive" },
    ...
    { title: "Ticket Médio", value: formatCurrency(ticketMedio), icon: DollarSign, color: "text-muted-foreground" },
